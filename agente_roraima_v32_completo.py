...
Agente Roraima v3.3
════════════════════════════════════════════════════
- Reglas primero para preguntas obvias (más fiable)
- LLM solo para preguntas ambiguas
- 19 divisas desde Google Sheets
- Memoria de largo plazo (SQLite thread-safe)
- Normalización y limpieza de parámetros del LLM
- System Prompt con ejemplos concretos
- Logging profesional
- Config centralizada
"""
...
class RegistroHerramientas:
    ...
    def _normalizar_params(self, herramienta: str, params: Dict) -> Dict:
        ...
        if herramienta == "calculadora":
            for alias in ["expression", "expr", "operacion", "formula", "calculo"]:
        ...
        if herramienta == "rag_buscar":
            for alias in ["query", "consulta", "busqueda", "search", "texto"]:
...
def calculadora(expresion: str) -> str:
    try:
        ...
        if match_descuento:
            ...
            return (
                f"Monto original: {monto:.2f} EUR\n"
                f"Descuento ({porcentaje}%): -{descuento:.2f} EUR\n"
                f"Total: {resultado:.2f} EUR"
            )
...
def convertir_moneda(monto: float, desde: str, hasta: str) -> str:
    ...
    desde = MAPA_PAISES_COMPLETO.get(str(desde).lower().strip(), str(desde).lower().strip())
    hasta = MAPA_PAISES_COMPLETO.get(str(hasta).lower().strip(), str(hasta).lower().strip())
    ...
    return f"Moneda no soportada. Disponibles: {', '.join(sorted(tasas.keys()))}"
...
def rag_buscar(pregunta: str) -> str:
    ...
    if any(p in pregunta_lower for p in [
        ...
        'mandar', 'requisito', 'necesito para', 'como mando',
    ...
    # Horarios
    if any(p in pregunta_lower for p in [
        'horario', 'abren', 'cierran', 'hora', 'abierto',
        'cerrado', 'domingo', 'sabado', 'festivo'
    ]):
        return horarios()

    # Servicios
    if any(p in pregunta_lower for p in [
        'servicio', 'ofrecen', 'delivery', 'recarga',
    ...
    for pais in MAPA_PAISES_COMPLETO.keys():
        if pais in pregunta_lower and len(pais) > 3:
    ...
    return (
        ...
        "envios de dinero y cotizaciones de divisas."
    )


# Registrar herramientas
registro.registrar("calculadora",     calculadora,
    "Calcula operaciones matematicas. Params: expresion")
registro.registrar("convertir_moneda", convertir_moneda,
    "Convierte EUR a monedas latinoamericanas. Params: monto, desde, hasta")
registro.registrar("rag_buscar",      rag_buscar,
    "Busca informacion interna del negocio. Params: pregunta")
registro.registrar("clima",           clima,
    "Obtiene el clima actual. Params: ciudad")
registro.registrar("noticias",        noticias,
    "Obtiene noticias recientes. Params: pais (opcional), categoria (opcional)")
registro.registrar("horarios",        horarios,
    "Devuelve el horario de atencion. Sin params")
registro.registrar("servicios",       servicios,
...
class MemoriaLargoPlazo:
    ...
    def recordar_hecho(self, usuario, clave, valor):
        with self._get_connection() as conn:
            ...
            if cursor.rowcount == 0:
                conn.execute(
                    "INSERT INTO hechos (usuario, clave, valor, timestamp) "
                    "VALUES (?,?,?,?)",
    ...
    def historial_usuario(self, usuario, ultimas_n: int = 5) -> List:
        with self._get_connection() as conn:
            cursor = conn.execute(
...
# ============================================================
# SYSTEM PROMPT (para preguntas ambiguas)
# ============================================================
...
3. Sé breve y amable. Responde en español
...
100 euros a colombia → {{"tipo":"herramienta","nombre":"convertir_moneda","parametros":{{"monto":"100","desde":"eur","hasta":"cop"}}}}
...
hnl=Honduras, nio=Nicaragua, gtq=Guatemala, mad=Marruecos, pkr=Pakistan
...
class BucleAgente:
    ...
    def pensar_con_llm(self, pregunta: str) -> Optional[Dict]:
        ...
        contexto = ""
        if hechos:
            contexto += f"Datos usuario: {json.dumps(hechos, ensure_ascii=False)}\n"
        if historial:
    ...
    def pensar(self, pregunta: str) -> Dict:
        """
        CAPA 1: Reglas deterministas para preguntas obvias
        CAPA 2: LLM para preguntas ambiguas
        CAPA 3: RAG como fallback final
        """
        p = pregunta.lower()

        # ── Memoria: recordar nombre ──────────────────────────
        if any(x in p for x in [
            'como me llamo', 'recuerdas mi nombre', 'recuerdas como',
            'sabes mi nombre', 'quien soy', 'mi nombre',
            'receurdas', 'recuerda mi', 'sabes como'
        ]):
            hechos = memoria.obtener_hechos(self.usuario_actual)
            nombre = hechos.get('nombre')
            if nombre:
                return {"tipo": "respuesta", "contenido": f"Te llamas {nombre}"}
        ...
        if 'me llamo' in p or 'mi nombre es' in p:
            ...
            if len(partes) > 1:
                nombre_raw = re.sub(r'[^a-z\s]', '', partes[1].strip()).strip()
                if nombre_raw:
        ...
        if any(x in p for x in [
            ...
            return {"tipo": "herramienta", "nombre": "rag_buscar",
                    "parametros": {"pregunta": pregunta}}

        # ── Servicios generales ───────────────────────────────
        if any(x in p for x in [
            'servicio', 'ofrecen', 'que hacen', 'que tienen',
            'a que se dedican', 'domicilio', 'delivery',
            'recarga', 'recargas', 'impresion', 'tienda online'
        ]):
            return {"tipo": "herramienta", "nombre": "servicios", "parametros": {}}

        # ── Descuentos ────────────────────────────────────────
        if 'descuento' in p:
            return {"tipo": "herramienta", "nombre": "calculadora",
                    "parametros": {"expresion": pregunta}}
        ...
        # ── Noticias ──────────────────────────────────────────
        if any(x in p for x in [
            'noticia', 'titular', 'news', 'prensa',
            'que paso', 'novedades'
        ]):
            return {"tipo": "herramienta", "nombre": "noticias",
                    "parametros": {}}
...
class AgenteRoraima:
    ...
    def procesar(self, pregunta: str) -> str:
        try:
            ...
            herramienta = decision.get("nombre", decision.get("tipo", "?"))
            ...
            return "Disculpa, tuve un problema. Puedes repetir la pregunta?"
...
if __name__ == "__main__":
    print("\n" + "=" * 55)
    print("   AGENTE RORAIMA v3.3")
    print("   Multiservicios Roraima | Mungia, Bizkaia")
    print("=" * 55)
    print(f"Estado LLM : {'Ollama activo' if cerebro.disponible else 'Modo reglas'}")
    ...
    while True:
        ...
        if pregunta.lower() in ["salir", "exit", "quit", "bye", "adios"]:
